...
class CPDatasetTest(data.Dataset):
    ...
    def get_agnostic(self, img, parse, pose_data):
            parse_array = parse
    ...
    def __getitem__(self, index):
        ...
        c = {}
        cm = {}
        for key in ['unpaired']:#self.c_names:
            c_name[key] = self.c_names[key][index]
            c[key] = Image.open(osp.join(self.data_path, 'cloth', c_name[key])).convert('RGB')
            c[key] = transforms.Resize(self.fine_width, interpolation=2)(c[key])
            cm[key] = Image.open(osp.join(self.data_path, 'cloth-mask', c_name[key].replace(c_name[key][-4:], ".png")))
        ...
        # person image
        im_pil_big = Image.open(osp.join(self.data_path, 'image', im_name)).convert("RGB")
        im_pil = transforms.Resize(self.fine_width, interpolation=2)(im_pil_big)

        im = self.transform(im_pil)

        # load parsing image
        parse_name_npy = im_name.replace("."+im_ext, '_vis2.npy')
        parse_name = im_name.replace("."+im_ext, "png")

        # 2d array with values from 0 to 15
        with open(osp.join(self.data_path, 'image-parse-v3', parse_name_npy), 'rb') as f:
            im_parse_pil_big = np.load(f)
        # im_parse_pil = transforms.Resize(self.fine_width, interpolation=0)(torch.from_numpy(im_parse_pil_big))
        parse = torch.from_numpy(np.array(im_parse_pil_big)[None]).long()
        # im_parse = self.transform(im_parse_pil)
        ...
        with open(osp.join(self.data_path, 'image-parse-agnostic-v3.2', parse_name_npy), 'rb') as f:
            image_parse_agnostic = np.load(f)
        # image_parse_agnostic = transforms.Resize(self.fine_width, interpolation=0)(torch.from_numpy(image_parse_agnostic))
        parse_agnostic = torch.from_numpy(np.array(image_parse_agnostic)[None]).long()
        # image_parse_agnostic = self.transform(image_parse_agnostic)
        ...
        # parse cloth & parse cloth mask for the upper body only of index 3. get cloth and make everything else white
        pcm = new_parse_map[3:4]
        im_c = im * pcm + (1 - pcm)

        # load pose points
        # pose_name = im_name.replace("."+im_ext, '_rendered.png')
        pose_name = im_name.replace("."+im_ext, '.png')
        ...
        # pose_name = im_name.replace("."+im_ext, '_keypoints.json')
        pose_name = im_name.replace("."+im_ext, '.npy')
        # with open(osp.join(self.data_path, 'openpose_json', pose_name), 'r') as f:
        #     pose_label = json.load(f)
        #     pose_data = pose_label['people'][0]['pose_keypoints_2d']
        #     pose_data = np.array(pose_data)
        #     pose_data = pose_data.reshape((-1, 3))[:, :2]
        with open(osp.join(self.data_path,"openpose_json",pose_name), 'rb') as f:
            pose_data = np.load(f)
            pose_data = pose_data.reshape((-1, 3))[:, :2]
