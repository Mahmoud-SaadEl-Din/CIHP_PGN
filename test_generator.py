...
from utils_viton.networks import ConditionGenerator, load_checkpoint, make_grid
from utils_viton.network_generator import SPADEGenerator
from tensorboardX import SummaryWriter
from utils_viton.utils import *
...
def get_opt():
    ...
    parser.add_argument("--fine_width", type=int, default=800)
    parser.add_argument("--fine_height", type=int, default=800)
...
def eval(opt, test_loader, tocg, generator):
    ...
    if opt.output_dir is not None:
        ...
        output_dir = os.path.join('./output','generator', 'output')
    grid_dir = output_dir#os.path.join('./output', 'generator', 'grid')
    ...
    with torch.no_grad():
        for inputs in test_loader.data_loader:
            ...
            if opt.cuda :
                ...
                pre_clothes_mask = torch.FloatTensor((pre_clothes_mask.detach().cpu().numpy() > 0.5))
            ...
            # forward
            flow_list, fake_segmap, warped_cloth_paired, warped_clothmask_paired = tocg(opt,input1, input2) # +++++++++++++++++++++++++++++ tocg
            
            # warped cloth mask one hot
            if opt.cuda :
                warped_cm_onehot = torch.FloatTensor((warped_clothmask_paired.detach().cpu().numpy() > 0.5).astype(np.float)).cuda()
            else :
                warped_cm_onehot = torch.FloatTensor((warped_clothmask_paired.detach().cpu().numpy() > 0.5))
...
def infer_hr_viton(data_root, out_, pairs_txt):
    prepare_samples(data_root)
    opt = get_opt()
    opt.dataroot = data_root
    opt.data_list = pairs_txt
    opt.output_dir = out_
    opt.fine_width = 768
    ...
    # Train
    eval(opt, test_loader, tocg, generator)
